CREATE OR REPLACE FUNCTION public.resolved_interview_slots_for_branding(_branding_id uuid)
 RETURNS TABLE(appointment_id uuid, appointment_date date, appointment_time time without time zone, slot_index integer)
 LANGUAGE sql
  ...
  auto_resolved AS (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE NOT (candidate.slot_index = ANY(COALESCE(m.taken, ARRAY[]::integer[])))
        AND COALESCE(lane.disabled, p.disabled, false) = false
        ...
        AND a.appointment_time >= COALESCE(
          (lane.day_time_overrides -> extract(isodow from a.appointment_date)::integer::text ->> 'start')::time,
          CASE WHEN extract(isodow from a.appointment_date)::integer IN (6, 7)
            THEN lane.weekend_start_time END,
          lane.start_time,
          (p.day_time_overrides -> extract(isodow from a.appointment_date)::integer::text ->> 'start')::time,
          CASE WHEN extract(isodow from a.appointment_date)::integer IN (6, 7)
            THEN p.weekend_start_time END,
          p.start_time
        )
        AND a.appointment_time < COALESCE(
          (lane.day_time_overrides -> extract(isodow from a.appointment_date)::integer::text ->> 'end')::time,
          CASE WHEN extract(isodow from a.appointment_date)::integer IN (6, 7)
            THEN lane.weekend_end_time END,
          lane.end_time,
          (p.day_time_overrides -> extract(isodow from a.appointment_date)::integer::text ->> 'end')::time,
          CASE WHEN extract(isodow from a.appointment_date)::integer IN (6, 7)
...
CREATE OR REPLACE FUNCTION public.book_interview_public(_application_id uuid, _appointment_date date, _appointment_time time without time zone)
...
BEGIN
  ...
  v_dow := extract(isodow from _appointment_date)::integer::text;
  ...
  LEFT JOIN public.branding_schedule_settings lane
   ...
   AND lane.slot_index = candidate.slot_index
  CROSS JOIN primary_settings p
  WHERE COALESCE(lane.disabled, p.disabled, false) = false
    AND extract(isodow from _appointment_date)::integer = ANY(COALESCE(lane.available_days, p.available_days))
    AND _appointment_time >= COALESCE(
      (lane.day_time_overrides -> v_dow ->> 'start')::time,
      CASE WHEN extract(isodow from _appointment_date)::integer IN (6, 7)
        THEN lane.weekend_start_time END,
      lane.start_time,
      (p.day_time_overrides -> v_dow ->> 'start')::time,
      CASE WHEN extract(isodow from _appointment_date)::integer IN (6, 7)
        THEN p.weekend_start_time END,
      p.start_time
    )
    AND _appointment_time < COALESCE(
      (lane.day_time_overrides -> v_dow ->> 'end')::time,
      CASE WHEN extract(isodow from _appointment_date)::integer IN (6, 7)
        THEN lane.weekend_end_time END,
      lane.end_time,
      (p.day_time_overrides -> v_dow ->> 'end')::time,
      CASE WHEN extract(isodow from _appointment_date)::integer IN (6, 7)
        THEN p.weekend_end_time END,
      p.end_time
    )
